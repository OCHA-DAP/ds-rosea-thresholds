...
def classify_hotspots(df):
    _df = df.copy()
    _df['consecutive_count'] = _df.groupby(['asap0_id', (_df['hs_code'] != _df['hs_code'].shift()).cumsum()])['hs_code'].cumcount()
    _df["alert_level"] = _df.apply(_classify_row, axis=1)
    _df = _df[["asap0_name", "date", "alert_level", "comment"]].rename(columns={"asap0_name": "country"})
    return _df