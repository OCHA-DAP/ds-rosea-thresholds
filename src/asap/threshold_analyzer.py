...
from .config import (
    ...
    WARNING_LEVEL_HIERARCHY, WARNING_THRESHOLDS, WARNING_COLUMNS,
...
class ThresholdAnalyzer:
    ...
    def _calculate_exposure_for_type(self, warnings_with_pop: pd.DataFrame, warning_type: str) -> pd.DataFrame:
        ...
        for (country, month), group in warnings_with_pop.groupby([WARNINGS_COUNTRY_COL, 'year_month']):
            result = {
                'country': country,
            ...
            for warning_level, pop in warning_level_groups.items():
                col_name = f'{warning_type}_pop_{warning_level.lower().replace(" ", "_")}'
                result[col_name] = pop
    ...
    def calculate_monthly_exposure(self) -> pd.DataFrame:
        ...
        logger.info("Calculating monthly population exposure for both crop and range warnings...")
        ...
        for warning_type in WARNING_COLUMNS.keys():
            ...
            for threshold in WARNING_THRESHOLDS:
                ...
                if col in self.monthly_exposure.columns:
                    stats[f'{warning_type}_warning_stats'][f'avg_pop_warning_{threshold}_plus'] = int(
                    ...
                    if pct_col in self.monthly_exposure.columns:
                        stats[f'{warning_type}_warning_stats'][f'avg_pct_warning_{threshold}_plus'] = round(
                            self.monthly_exposure[pct_col].mean(), 2
                        )
